...
```
fotdata-api/
├── main.py                 # FastAPI 백엔드 (766줄) — 전체 API 엔드포인트
├── update_data.py          # 데이터 수집 + 피처 생성 + 모델 학습 + 파생 산출물 생성 (1,219줄)
├── FotData.html            # 메인 웹앱 (3,771줄, 탭: 경기예측/순위예측/순위표(+UCL 리그스테이지·토너먼트)/일정/선수/승부차기, 모달: 팀정보(개요·순위·경기·스쿼드·플레이어통계(준비중)·팀통계·이적)/경기예측 미리보기/트랙레코드/전역 팀검색(Ctrl·⌘K)/토너먼트 대진, 위젯: 빅매치·오늘의(다음) 경기·내 팀(즐겨찾기, localStorage)·PWA 설치 배너·Ko-fi 후원)
...
└── fotdata_model/            # 모델/데이터 산출물 (전부 update_data.py가 생성·갱신)
    ...
    ├── team_state.json          # 팀별 "오늘 시점" 모델 입력값(누적 ELO·최근 5경기 승점·최근 10경기 득실·최근 38경기 공격/수비/승률) — /predict 입력 (2026-09-27 추가, 5.3 참고)
