...
- **경기 데이터 누락 (2026-09-27 발견, 미해결)**: `all_matches.csv`의 24-25 시즌이 리그별로 빠져 있음 — BL1 272/306, FL1 255/306, PD 323/380, SA 323/380(PL만 380 완전). 23-24 시즌은 PL·PD·CL만 있고 BL1·SA·FL1은 없음. 예: 24-25 뮌헨–도르트문트 2경기 누락(그래서 맞대결이 2경기뿐). 원인: `main()`이 매일 2024~2026 시즌을 받아오지만 26-27(2026-08-01 이후)만 새 데이터로 교체하고 그 이전은 "기존 CSV 유지"라, 과거 어느 시점에 불완전하게 받아진 24-25가 그대로 굳어버림. 해결 방향: 과거 시즌도 새로 받은 데이터와 합집합으로 병합(받아온 쪽 우선) + 2023 시즌도 수집(무료 플랜 4시즌 접근 가능) → 모델 학습 데이터·ELO·맞대결 모두 영향이라 재학습 후 시간순 검증 비교 필요. 맞대결 제목 "최근 4시즌"도 이 수정 후에야 5개 리그 전부에 정확해짐.
